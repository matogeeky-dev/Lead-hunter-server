...
async function geocodeLocation(location) {
  ...
  const candidates = data.map(hit => {
    ...
    return {
      ...
      // 2-letter country code (e.g. "us", "ng", "gb") from Nominatim's own
      // address breakdown — used to set Google's `gl` param per-search so
      // results are regionally biased toward the actual target country
      // instead of defaulting to wherever the server's IP happens to be.
      countryCode: (hit.address && hit.address.country_code) || null,
...
// launchBrowser's own steps after chromium.launch() (newContext, addCookies,
// addInitScript, route) had no timeout at all. If the browser process comes
// up half-alive — plausible on a resource-starved container recovering from
// a crash — browser.newContext() can hang forever with nothing to stop it.
// Every recycle path awaits launchBrowser() directly, so that hang used to
// become a silent, permanent freeze: every other worker just sits in a
// `while (job._recycling)` loop waiting for a recovery that never finishes,
// with no new log lines and no error, indistinguishable from the job still
// working. Wrapping the whole function in a hard deadline turns that into a
// normal, catchable, loggable error instead — nothing downstream can ever
// wait on this longer than LAUNCH_DEADLINE_MS again.
const LAUNCH_DEADLINE_MS = 90000;
async function launchBrowserOnce() {
  return withTimeout(_launchBrowserInner(), LAUNCH_DEADLINE_MS, "launchBrowser").catch(async e => {
    // If we timed out mid-launch, don't leak a half-alive Chromium process —
    // there's no clean handle to it here, so best-effort kill any stray
    // chromium process this container spawned recently. safeCloseBrowser
    // can't help since we don't have a browser object when newContext (not
    // launch) is what hung.
    try {
...
// A single bad relaunch (transient Docker/Railway hiccup right as we're
// recovering from the LAST crash) used to take the whole job down and lose
// everything found so far, even mid-run with thousands of good results
// already in hand. Three attempts with backoff before finally giving up.
async function launchBrowser(job) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastErr = e;
      if (job) jlog(job, `   ⚠️ Browser (re)launch attempt ${attempt}/3 failed (${e.message}).${attempt < 3 ? " Retrying…" : ""}`);
      if (attempt < 3) await sleep(3000 * attempt);
    }
  }
  throw lastErr;
}
async function _launchBrowserInner() {
  let browser;
  try {
    browser = await chromium.launch({
      headless: true,
  ...
  // Every browser recycle creates a brand-new context with zero cookies,
  // which means Google's "before you continue" consent interstitial shows
  // up again on the very next page load — and since nothing after the
  // original warmup page ever calls dismissDialogs(), every zone/detail
  // page hit after the first recycle silently loads the consent wall
  // instead of the real content and comes back empty. Setting this cookie
  // up front makes every fresh context pre-consented, so the wall never
  // appears in the first place.
  await context.addCookies([
    { name: "CONSENT", value: "YES+cb.20220301-11-p0.en+FX+666", domain: ".google.com", path: "/" },
    // Google's consent flow has shifted to also check this cookie in some
    // regions/flows — belt-and-suspenders alongside CONSENT above.
...
async function deepScroll(page, cellCap, job) {
  try {
    await page.waitForSelector('a[href*="/maps/place/"]', { timeout: 12000 });
  } catch (_) {
    // The list-view selector never showed up. Two different things cause
    // this: a genuine "no results" zone / consent-or-CAPTCHA page, OR —
    // common for narrow/specific zones — Google skipping the list entirely
    // and redirecting straight to the one matching business's own page. The
    // second case still has a real lead sitting right there in the DOM, just
    // not in list-item form, so pull it out directly instead of losing it.
    if (/\/maps\/place\//.test(page.url())) {
      try {
        const single = await page.evaluate(() => {
          ...
          const nameIdx = lines.findIndex(l => l === title);
          let address = "";
          if (nameIdx >= 0) {
            for (let i = nameIdx + 1; i < Math.min(nameIdx + 8, lines.length); i++) {
              ...
              if (/permanently closed/i.test(line)) continue;
              if (line.length >= 4) { address = line; break; }
            }
          }
          return { name: title, address, category: "" };
        });
        if (single && single.name) {
          if (job) jlog(job, `   ↪️ Zone redirected straight to a single business page — captured "${single.name}" directly.`);
          return [{ ...single, mapsUrl: page.url() }];
        }
      } catch (_) { /* fall through to diagnostic below */ }
    }
    // Logged once per job (not per zone) so a bad run doesn't flood the log.
    if (job && !job._loggedEmptyZoneDiag) {
      job._loggedEmptyZoneDiag = true;
      try {
        const title = await page.title();
        const snippet = await page.evaluate(() => (document.body?.innerText || "").replace(/\s+/g, " ").trim().slice(0, 300));
        const url = page.url();
        jlog(job, `   🔬 Diagnostic — empty zone page. URL: ${url} | Title: "${title}" | Text: "${snippet}"`);
      } catch (diagErr) {
        jlog(job, `   🔬 Diagnostic capture itself failed: ${diagErr.message}`);
      }
    }
    return [];
  }
...
async function getPlacePhone(page, mapsUrl) {
  try {
    // Belt-and-suspenders: force English even if the scraped place URL
    // didn't carry hl/gl through from the search results page.
    const sep = mapsUrl.includes("?") ? "&" : "?";
    await gotoWithRetry(page, `${mapsUrl}${sep}hl=en`);
    await sleep(1100);
    return await page.evaluate(() => {
      let phone = "";
...
// If still critical after waiting, we force a real recycle (kill the
// browser, launch a fresh one) via the caller-supplied `recycle` callback
// instead of opening another tab into an already-full container.
//
// IMPORTANT: `job._recycling` alone only stops NEW workers from starting —
// it does nothing to protect workers that are already mid-flight, using
// pages from the browser that's about to be torn down. Calling
// safeCloseBrowser() while those are still active is exactly what produced
// "Target page, context or browser has been closed" errors. So before the
// actual recycle, we drain: wait for job._inFlightPages (every worker
// currently holding an open page, tracked via acquirePageSlot/
// releasePageSlot around their page usage) to reach zero, with a safety cap
// in case something's stuck despite the timeout wrappers elsewhere.
// Nothing should ever wait on job._recycling longer than this. launchBrowser
// now has its own internal deadline (see LAUNCH_DEADLINE_MS above) so this
// should never actually get hit — but it's a second, independent backstop:
// if any future change introduces another way for a recycle to hang, workers
// give up waiting and force the flag clear themselves rather than freeze the
// job forever with no way to recover, which is what happened here.
const RECYCLE_WAIT_CEILING_MS = 150000;
async function waitOutRecycling(job) {
  const start = Date.now();
  while (job._recycling) {
    if (Date.now() - start > RECYCLE_WAIT_CEILING_MS) {
      jlog(job, `   🆘 A recycle has been stuck for over ${Math.round(RECYCLE_WAIT_CEILING_MS / 1000)}s — forcing it clear so the job can keep moving.`);
...
async function waitForMemoryHeadroom(job, label, recycle) {
  // If a recycle is already in progress (started by another concurrent
  // worker), don't start new work until it's finished with a fresh browser.
  await waitOutRecycling(job);
  ...
  job._recycling = true;
  try {
    jlog(job, `   🚨 Memory still critical (${mem.usageMB}MB/${mem.limitMB}MB) after waiting — draining in-flight tabs before an emergency recycle…`);
    const drainStart = Date.now();
    while ((job._inFlightPages || 0) > 0 && Date.now() - drainStart < 25000) await sleep(300);
    if ((job._inFlightPages || 0) > 0) {
      jlog(job, `   ⚠️ ${job._inFlightPages} tab(s) still in flight after 25s — recycling anyway (they'll error out and get skipped, not crash the job).`);
    }
    jlog(job, `   🔁 Recycling browser now…`);
    await recycle();
    jlog(job, `   ✅ Recycled — resuming.`);
  } finally {
    job._recycling = false;
  }
}

// Wrap every worker's "open a page, use it, close it" span with these so the
// drain above knows what's actually in flight. acquire pauses (rather than
// proceeding) if a recycle is currently underway, so nothing starts fresh
// work against a browser that's mid-teardown.
async function acquirePageSlot(job) {
  await waitOutRecycling(job);
  job._inFlightPages = (job._inFlightPages || 0) + 1;
}
function releasePageSlot(job) {
  job._inFlightPages = Math.max(0, (job._inFlightPages || 0) - 1);
}
  ...
  try {
    ...
    if (location) {
      ...
      try {
        ...
        zoom = grid.zoom;
        job.countryCode = geo.countryCode; // used for Google's `gl` param below
    ...
    let { browser: b, context } = await launchBrowser(job);
    browser = b;
    ...
    // zone 0 alone keeps that logic simple before fanning out in parallel.
    {
      ...
      try {
        ...
        // hl forces English UI; gl biases results toward the *actual* target
        // country (from geocoding), not a hardcoded one — without either,
        // Google infers both from the server's IP (Amsterdam), which was
        // serving Dutch "no results" pages and drifting results toward the
        // Netherlands regardless of the requested @lat,lon.
        const langParams = `hl=en${job.countryCode ? `&gl=${job.countryCode}` : ""}`;
        const searchUrl = lat != null
          ? `https://www.google.com/maps/search/${encodeURIComponent(searchTerm)}/@${lat},${lon},${zoom}z?${langParams}`
          : `https://www.google.com/maps/search/${encodeURIComponent(searchTerm + (location ? " in " + location : ""))}?${langParams}`;
        await gotoWithRetry(warmupPage, searchUrl);
        await sleep(1200);
        await dismissDialogs(warmupPage);
        const listings = await deepScroll(warmupPage, cellCap, job);
        let added = 0;
    ...
    while (zi < gridPoints.length && !job.cancelled && seen.size < maxResults) {
      ...
      await runPool(genPoints, zoneConcurrency, async (pt, localIdx) => {
        ...
        const searchUrl = pt.lat != null
          ? `https://www.google.com/maps/search/${encodeURIComponent(searchTerm)}/@${pt.lat},${pt.lon},${zoom}z?${langParams}`
          : `https://www.google.com/maps/search/${encodeURIComponent(searchTerm + (location ? " in " + location : ""))}?${langParams}`;
        await waitForMemoryHeadroom(job, "zone", async () => {
          await safeCloseBrowser(browser);
          ({ browser: b, context } = await launchBrowser(job));
          browser = b;
        });
        await acquirePageSlot(job);
        let zonePage;
        try {
          zonePage = await context.newPage();
          await gotoWithRetry(zonePage, searchUrl);
          await sleep(900);
          const listings = await deepScroll(zonePage, cellCap, job);
          let added = 0;
          ...
          if (zonePage) await withTimeout(zonePage.close(), 5000, "page.close").catch(() => {});
          releasePageSlot(job);
        }
        ...
        await runPool(genItems, detailConcurrency, async (listing) => {
          ...
          try {
            try {
              ...
              if (currentPage) await withTimeout(currentPage.close(), 5000, "page.close").catch(() => {});
              try { currentPage = await context.newPage(); } catch (_) { currentPage = null; }
            }
